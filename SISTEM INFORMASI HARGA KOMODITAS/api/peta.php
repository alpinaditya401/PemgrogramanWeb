    ...
    <div class="peta-wrap">
        ...
        <aside class="peta-side">
            <div class="h-14 flex items-center px-4 border-b border-[var(--border)] flex-shrink-0">
                <a href="dashboard-user.php" class="flex items-center gap-2">
                    <div
                        class="w-6 h-6 bg-brand-500 rounded-lg flex items-center justify-center shadow shadow-brand-500/30">
            ...
            <div class="px-4 py-3 border-b border-[var(--border)] flex-shrink-0">
                <label
                    class="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-1.5">Komoditas</label>
                <form method="GET">
            ...
            <div class="px-4 py-3 border-b border-[var(--border)] flex-shrink-0 space-y-1.5">
                ...
                <div class="flex justify-between text-xs">
                    ...
                    <span
                        class="font-bold text-brand-500"><?= ($minHarga && $minHarga !== PHP_INT_MAX) ? rupiah($minHarga) : '—' ?></span>
                </div>
            </div>

            <!-- Skala warna -->
            <div class="px-4 py-3 border-b border-[var(--border)] flex-shrink-0">
                <label class="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2">Skala
                    Harga</label>
                <canvas id="legendCanvas" height="10" style="width:100%;border-radius:4px"></canvas>
                <div class="flex justify-between mt-1">
                    <span
                        class="text-[9px] text-[var(--text-muted)]"><?= $minHarga && $minHarga !== PHP_INT_MAX ? rupiah($minHarga) : 'Rendah' ?></span>
                    <span
                        class="text-[9px] text-[var(--text-muted)]"><?= $maxHarga ? rupiah($maxHarga) : 'Tinggi' ?></span>
                </div>
                    ...
                    foreach ($petaData as $prov => $d): ?>
                        <a href="chart.php?provinsi=<?= urlencode($prov) ?>&komoditas=<?= urlencode($selKom) ?>"
                            ...
                            <div class="flex items-center gap-2 min-w-0">
                                <span
                                    class="text-[10px] text-[var(--text-muted)] w-4 text-right flex-shrink-0"><?= $rank++ ?></span>
                                <span
                                    class="text-[var(--text-secondary)] group-hover:text-[var(--text-primary)] transition truncate"><?= htmlspecialchars($prov) ?></span>
                            </div>
                            <span
                                class="font-bold font-display text-[var(--text-primary)] flex-shrink-0 ml-2 text-[11px]"><?= rupiah($d['avg']) ?></span>
                        </a>
            ...
            <div class="border-t border-[var(--border)] px-3 py-2 space-y-0.5 sidebar-nav flex-shrink-0">
                <a href="dashboard-user.php" class="text-xs"><i data-lucide="arrow-left" class="w-3.5 h-3.5"></i>
                    Dashboard</a>
                <a href="compare.php" class="text-xs"><i data-lucide="git-compare" class="w-3.5 h-3.5"></i>
                    Perbandingan</a>
                <a href="#" data-action="toggle-theme" class="text-xs"><i data-lucide="moon" data-theme-icon="toggle"
                        class="w-3.5 h-3.5"></i> Ganti Tema</a>
            </div>
        </aside>
        ...
        <div class="peta-main">
            ...
            <div class="peta-body">
                <!-- Loading state -->
                <div id="mapLoading"
                    class="absolute inset-0 flex items-center justify-center z-30 bg-[var(--bg-primary)]">
                    <div class="text-center">
                        <div class="w-12 h-12 border-2 border-brand-500 border-t-transparent rounded-full mx-auto mb-3"
            ...
            g.selectAll('.province')
                ...
                .on('mousemove', function (event, d) {
                    ...
                    if (data) {
                        ...
                        ttDetail.innerHTML =
                            '<div style="font-size:10px">Kontributor belum melaporkan harga di wilayah ini.</div>';
                    }
                ...
                .on('click', (event, d) => {
                    ...
                    if (provName) {
                        window.location.href =
                            `chart.php?provinsi=${encodeURIComponent(provName)}&komoditas=${encodeURIComponent(SEL_KOM)}`;
                    }
                });
        }
