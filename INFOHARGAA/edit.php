<?php
/**
 * edit.php — Admin Edit Data Komoditas
 */
session_start();
...
if (isset($_POST['update_data'])) {
    ...
    $conn->query("UPDATE komoditas SET nama='$nama',kategori='$kategori',lokasi='$lokasi',
                  provinsi='$provinsi',satuan='$satuan',harga_kemarin=$kemarin,harga_sekarang=$sekarang,
                  history='$hj' WHERE id=$id");
...
<body class="min-h-screen flex items-center justify-center p-4">
  ...
  <!-- Ambient -->
  <div class="fixed top-1/3 left-1/2 -translate-x-1/2 w-80 h-80 bg-brand-500/5 rounded-full blur-[100px] pointer-events-none"></div>

  <div class="w-full max-w-lg relative z-10">

    <!-- Back button -->
    <a href="dashboard.php"
       class="flex items-center gap-1.5 text-sm text-[var(--text-muted)] hover:text-[var(--text-primary)] transition mb-5 group">
      <i data-lucide="arrow-left" class="w-4 h-4 group-hover:-translate-x-0.5 transition-transform"></i>
      Kembali ke Dashboard
    </a>

    <div class="card overflow-hidden shadow-2xl animate-fade-up">
      ...
      <div class="px-6 py-4 border-b border-[var(--border)] bg-[var(--bg-secondary)] flex items-center gap-3">
        ...
        <div>
          <h1 class="font-display font-bold text-[var(--text-primary)]">Edit Data Komoditas</h1>
          <p class="text-[10px] text-[var(--text-muted)]">ID #<?= $id ?> &bull; <?= htmlspecialchars($row['nama']) ?></p>
        </div>
        <!-- Theme toggle -->
        <button data-action="toggle-theme"
                class="ml-auto w-8 h-8 flex items-center justify-center rounded-lg bg-[var(--surface)] hover:bg-[var(--surface-hover)] text-[var(--text-muted)] transition">
      ...
      <form action="edit.php?id=<?= $id ?>" method="POST" class="px-6 py-5 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <select name="kategori" class="input-field">
              ...
              <option <?= $row['kategori']===$k?'selected':'' ?>><?= htmlspecialchars($k) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div>
            ...
            <select name="satuan" class="input-field">
              ...
              <option <?= $row['satuan']===$s?'selected':'' ?>><?= htmlspecialchars($s) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="lokasi" class="input-field"
          ...
          <div>
            ...
            <select name="provinsi" class="input-field">
              ...
              <option value="<?= htmlspecialchars($p) ?>" <?= $row['provinsi']===$p?'selected':'' ?>>
                <?= htmlspecialchars($p) ?>
              </option>
              <?php endforeach; ?>
            </select>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" name="kemarin" class="input-field"
                   value="<?= (int)$row['harga_kemarin'] ?>" required min="0"/>
          </div>
          <div>
            <label class="block text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider mb-1.5">Harga Sekarang (Rp)</label>
            <input type="number" name="sekarang" class="input-field"
                   value="<?= (int)$row['harga_sekarang'] ?>" required min="0"/>
          </div>
        </div>

        <!-- Info note -->
        <div class="flex items-start gap-2 p-3 rounded-xl bg-brand-500/6 border border-brand-500/15 text-xs text-[var(--text-secondary)]">
          <i data-lucide="info" class="w-3.5 h-3.5 text-brand-500 flex-shrink-0 mt-0.5"></i>
          Harga Sekarang akan otomatis ditambahkan ke riwayat grafik 7 hari.
        </div>

        <!-- Actions -->
        <div class="flex gap-3 pt-2">
          <a href="dashboard.php"
             class="flex-1 py-2.5 rounded-xl text-sm font-semibold text-center text-[var(--text-secondary)] bg-[var(--surface)] hover:bg-[var(--surface-hover)] transition">
            Batal
          </a>
          <button type="submit" name="update_data"
                  class="flex-1 py-2.5 rounded-xl text-sm font-display font-bold text-white bg-brand-600 hover:bg-brand-500 transition shadow shadow-brand-600/20 hover:-translate-y-0.5">
            Simpan Perubahan
          </button>
        </div>
      </form>
