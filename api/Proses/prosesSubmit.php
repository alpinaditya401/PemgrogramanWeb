 ...
 * Kontributor kirim laporan harga → status pending
 */
...
$hist = esc($conn, json_encode(array_fill(0, 6, $kemarin) + [6 => $sekarang]));

// ✅ FIX: pakai kolom yang ada di schema (kategori, history, submitted_by, user_id semuanya ada)
$ok = $conn->query("INSERT INTO komoditas
    (nama, kategori, lokasi, provinsi, satuan, harga_kemarin, harga_sekarang, history, status, submitted_by, user_id, catatan)
    VALUES
    ('$nama','$kategori','$lokasi','$provinsi','$satuan',$kemarin,$sekarang,'$hist','pending',$uid,$uid,'$catatan')");

if (!$ok) redirect('/dashboard-user.php?error=db&detail=' . urlencode($conn->error));
